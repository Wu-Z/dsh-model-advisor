...
| 位置 | 内容 |
|---|---|
| 侧边栏左下角（设置按钮上方） | 三段：点 `余额：¥45` **刷新余额**；中间一个**峰谷小图案**（太阳＝峰时／月亮＝谷时，悬停显示「至北京时间 X（还有 Y）」；只在 DeepSeek 路由下出现）；点右侧 `▸` **打开面板**；**侧边栏收起时只显示余额数字**（图案与按钮都隐藏，需要先展开侧边栏才能打开面板） |
...
- **长上下文的阶梯价未处理**：上游有 441 条（6%）在超过阈值后提价（例如 gemini-3.1-pro 超 200K 输入 $2→$4、输出 $12→$18），面板只显示基础档；跑长上下文时实际成本会高于显示值。
- 音频计费（130 条，2%）与知识截止时间（4006 条，53%）已在上游提供但未展示。
